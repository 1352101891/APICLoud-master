...
public class PcLiveLandscapeViewManager {
    ...
    public void init() {
        ...
        initDefinitionPopup();
        mTitle.setText(dwLive.getRoomInfo().getName());
    }
    ...
    public void onClick(View view) {
        ...
        // TODO 待测试
        if (mContext instanceof PcLivePlayActivity) {
            //除去一下几种情况，其他的都不可以在连麦的状况下操作。
            if (((PcLivePlayActivity) mContext).isSpeaking && view.getId() != R.id.iv_portrait_live_full && view.getId() != R.id.iv_land_live_video_rtc && view.getId() != R.id.iv_phone_live_close) {
        ...
        } else if (i == R.id.iv_phone_live_close) { //返回键代表退出
            ((PcLivePlayActivity)mContext).onBackPressed();
        ...
        }else if(i== R.id.iv_portrait_live_full){
            if (((PcLivePlayActivity)mContext).isPrepared())
                ((PcLivePlayActivity)mContext).fullVideo();
        }
    }
    ...
    // 该函数是监听视频面板的点击事件的，返回值表示显示或者隐藏，true表示显示
    ...
    private void postDelayScreenDisappear() {
        handler.removeCallbacks(runnable);
        if (!isChat){ //如果在输入文字的时候，不允许让输入框消失
