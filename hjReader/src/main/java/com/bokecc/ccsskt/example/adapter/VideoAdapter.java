...
public class VideoAdapter extends BaseRecycleAdapter<VideoAdapter.LittleVideoViewHolder, VideoStreamView> {
    ...
    public final static float Video_rate=1.777f;
    public final static float layout_rate=0.3f;
    ...
    public VideoAdapter(Context context) {
        super(context);
        videoHeight= (int) (CCApplication.screenEntity.height*layout_rate/Video_rate);
        if (CCApplication.sClassDirection == 1) { // 横屏
            itemHeight = videoHeight+DensityUtil.dp2px(context, 4); //加上item间距
        }else {
            itemHeight = (int) (videoHeight*Video_rate+DensityUtil.dp2px(context, 4)); //加上item间距
        }
    }
...
//                itemView.getLayoutParams().height = DensityUtil.dp2px(mContext, 81);
                itemView.getLayoutParams().width = (int) (videoHeight* Video_rate);
                 itemView.getLayoutParams().height = videoHeight;
            }
        }
