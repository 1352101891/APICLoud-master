    ...
    public static boolean RequestStore(Context context) {
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            if (Permission_w != PackageManager.PERMISSION_GRANTED || Permission_r != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions((Activity) context, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE}, REQUEST_write_Permission);
