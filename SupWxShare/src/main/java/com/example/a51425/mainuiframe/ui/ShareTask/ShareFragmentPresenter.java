...
public class ShareFragmentPresenter extends BasePresenter2<ShareFragment,ShareLogic> {
    ...
    public ShareFragmentPresenter(ShareFragment fragment, ShareLogic model,String appid,String packagename) {
        super(fragment, model);
        shareAppId=appid;
        shareAppPackageName=packagename;
    }
        ...
        Observable.just(ShareUtils.shareWXReadyRx(shareImage))
                .filter(new Func1<String[], Boolean>() {
                    ...
                    public Boolean call(String[] strings) {
                        boolean isEmpty= shareAppId.equals("")&&shareAppPackageName.equals("");
                        if (!isEmpty ){
